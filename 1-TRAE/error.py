...
def linearErrors(xs, ys, a_0, a_1, o_y = 0):
    o_y = max(sqrt(sum([(y - (a_0 + a_1 * x))**2 for (x, y) in zip(xs, ys)]) / (len(xs) - 2)), o_y)
    xs2 = [x * x for x in xs]
...
xs, ys = zip(*sorted(readFile("A1.txt")))
o_y = 0.03
a_0 = 0.00375
a_1 = 0.00217
da_0, da_1 = linearErrors(xs, ys, a_0, a_1, o_y)
print("A1:", da_0, ";", da_1)
